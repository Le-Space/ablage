 ...
 *
 * ```js
 * new Provider(doc, message => stream.send(encode(message)))
 * new Provider(doc, message => pubsub.publish(topic, encode(message)))
 * ```
 *
 * Keeping the protocol and the channel separable is most of the design here.
 * The message shapes are the hackathon provider's, so the two can be compared
 * on the wire.
 *
